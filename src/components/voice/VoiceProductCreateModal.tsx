 ...
 * e.g. "চিনি, কেনার দাম ১২০, বিক্রির দাম ১৩০, স্টক ৫০ কেজি"
 * e.g. "সয়াবিন তেল ১ লিটার, কেনা ১৮০, বেচা ১৯৫, ২০ বোতল"
 * e.g. "ডিম, কেনা ১১ টাকা, বেচা ১২ টাকা, ১০০ পিস"
 */
export function parseSpokenProduct(phrase: string): {
  ...
  let text = phrase.replace(/[০-৯]/g, (d) => bnToEn[d] || d)
  ...
  // Detect Unit
  if (/কেজি|kg|গ্রাম/i.test(text)) {
    unit = 'kg'
  } else if (/লিটার|litre|লি|ml/i.test(text)) {
    unit = 'litre'
  } else if (/প্যাকেট|packet/i.test(text)) {
    unit = 'packet'
  } else if (/ডজন|dozen/i.test(text)) {
    unit = 'dozen'
  } else if (/হালি/i.test(text)) {
    unit = 'hali'
  } else if (/বস্তা|ব্যাগ/i.test(text)) {
    unit = 'sack'
  }

  // Extract Buy Price (কেনার দাম / কেনা / ক্রয় / buy)
  const buyMatch = text.match(/(?:কেনার\s*দাম|কেনা|ক্রয়|ক্রয়মূল্য|buy)\s*(?:হলো|হল|ছিল|দাম)?\s*(\d+(?:\.\d+)?)/i)
  if (buyMatch && buyMatch[1]) {
    buyPrice = parseFloat(buyMatch[1])
  }

  // Extract Sell Price (বিক্রির দাম / বেচা / বিক্রি / sell)
  const sellMatch = text.match(/(?:বিক্রির\s*দাম|বেচা|বিক্রি|বিক্রয়|বিক্রয়মূল্য|sell)\s*(?:হলো|হল|দাম)?\s*(\d+(?:\.\d+)?)/i)
  if (sellMatch && sellMatch[1]) {
    sellPrice = parseFloat(sellMatch[1])
  }

  // Extract Stock (স্টক / পরিমাণ / মোট / আছে)
  const stockMatch = text.match(/(?:স্টক|পরিমাণ|আছে|মোট)?\s*(\d+(?:\.\d+)?)\s*(?:টি|টা|পিস|কেজি|লিটার|প্যাকেট|ডজন|হালি|বোতল|বস্তা|piece|kg)?\s*(?:স্টক|আছে)/i) ||
    text.match(/স্টক\s*(\d+(?:\.\d+)?)/i)
  if (stockMatch && stockMatch[1]) {
    stock = parseFloat(stockMatch[1])
  }

  // Clean Product Name: remove extracted price/stock phrases
  let cleanName = text
    .replace(/(?:কেনার\s*দাম|কেনা|ক্রয়|ক্রয়মূল্য|buy)\s*(?:হলো|হল|ছিল|দাম)?\s*\d+(?:\.\d+)?\s*(?:টাকা)?/gi, '')
    .replace(/(?:বিক্রির\s*দাম|বেচা|বিক্রি|বিক্রয়|বিক্রয়মূল্য|sell)\s*(?:হলো|হল|দাম)?\s*\d+(?:\.\d+)?\s*(?:টাকা)?/gi, '')
    .replace(/(?:স্টক|পরিমাণ)?\s*\d+(?:\.\d+)?\s*(?:টি|টা|পিস|কেজি|লিটার|প্যাকেট|ডজন|হালি|বোতল|বস্তা)?\s*(?:স্টক|আছে)/gi, '')
    .replace(/(?:টাকা|টাকার|দাম|হলো|হল)/gi, '')
    .replace(/[,;]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()

  return {
    name: cleanName || text.trim(),
    buyPrice,
    sellPrice,
    stock,
    unit,
